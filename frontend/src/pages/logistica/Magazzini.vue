            ...
            <div class="w-full max-w-md bg-[#0b1020] border border-white/10 rounded-xl p-4">
              ...
              <div class="mt-4 flex items-center gap-2">
                ...
                >
                  ...
                  <span v-else>Elimina magazzino</span>
                </button>
                <button
...
async function deleteWarehouse() {
  ...
  if (!confirm('Eliminare definitivamente questo magazzino?')) return;
  modal.value.deleting = true;
  modal.value.error = '';
  try {
    await api.delete(`/warehouses/${modal.value.id}`);
    modal.value.open = false;
    await load();
  } catch (e: any) {
    modal.value.error = e?.response?.data?.error || e?.message || 'Errore eliminazione magazzino';
  } finally {
